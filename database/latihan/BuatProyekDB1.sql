CREATE DATABASE IF NOT EXISTS book2;

USE book2;

CREATE TABLE IF NOT EXISTS sheet1 (
	saklar_lampu	VARCHAR(3)
);

  DELETE FROM sheet1;

ALTER TABLE sheet1
	DROP PRIMARY KEY;

ALTER  TABLE sheet1
	ADD PRIMARY KEY(saklar_lampu);

-- Memasukkan 2 record sekaligus
INSERT INTO sheet1 (saklar_lampu) VALUES
	('ON'),
	('OFF'); 

/*INSERT INTO sheet1 (saklar_lampu) VALUES
	('ON'),
	('OFF'); 
*/

/* debug untuk jumlah Record */
SELECT saklar_lampu as 'Nama Saklar Lampu'
	   /* Tidak Disarankan*/ ,
	   COUNT(saklar_lampu) as Status_Saklar_Lampu
FROM   sheet1
GROUP BY saklar_lampu
ORDER BY saklar_lampu ASC;

SELECT saklar_lampu AS 'SAKLAR LAMPU',
		IF(saklar_lampu='ON','Nyala','Mati') AS STATUS_LAMPU
FROM sheet1;

/* sheet 2 */

 CREATE TABLE IF NOT EXISTS sheet2 (
	lampu_lalin VARCHAR(6) PRIMARY KEY
);

DELETE FROM sheet2;

ALTER TABLE sheet2
	DROP PRIMARY KEY;


ALTER  TABLE sheet2
	ADD PRIMARY KEY(lampu_lalin);

INSERT INTO sheet2 VALUES
	('MERAH'),
	('KUNING'),
	('HIJAU');

SELECT lampu_lalin AS lampu_lalin
FROM sheet2
GROUP BY lampu_lalin;

 SELECT lampu_lalin as 'Lampu lalin',
	IF(lampu_lalin='MERAH', 
		'Berhenti',
		IF(lampu_lalin='KUNING',
			'Berhati-hati',
			'Jalan'
			)
		)
	 AS STATUS_LAMPU_LALIN
FROM sheet2;

			/* Record Ke 3 */

CREATE TABLE IF NOT EXISTS warga(
	nama VARCHAR(6),
	jenis_kelamin VARCHAR(6),
	status VARCHAR(7),
	PRIMARY KEY(nama,jenis_kelamin,status)
);
DELETE FROM warga;


/*ALTER  TABLE warga
	ADD PRIMARY KEY(nama,jenis_kelamin,status); */


INSERT INTO warga VALUES
	('Amin','Pria','sendiri'),
	('Rahman','Pria','kawin'),
	('Luki','Pria','Cerai'),
	('Dea','Wanita','sendiri'),
	('Dea','Pria','Kawin'),
	('Dea','Pria','Cerai'),
	('Yani','Wanita','kawin'),
	('Endah','Wanita','Cerai');
	
DROP VIEW vw_seluruh_warga;

CREATE VIEW vw_seluruh_warga AS
SELECT *
FROM warga;
SELECT 	nama,
		jenis_kelamin,
		status,
		IF(jenis_kelamin='Pria',
			IF(status='sendiri',
				'Perjaka',
				IF(status='Kawin',
					'Menikah',
					'Duda'
					)
				),
			IF(status='sendiri',
				'Perawan',
				IF(status='Kawin',
					'Menikah',
					'Janda'))
		)
			AS keterangan_IF,

		CASE WHEN jenis_kelamin='Pria'
			THEN 
			CASE WHEN status= 'sendiri'
				THEN  'Perjaka'
				ELSE
					CASE WHEN status='Kawin'
					THEN 'Menikah'
					ELSE 
					'Duda'
					END
			END
			ELSE

			CASE WHEN status= 'sendiri'
				THEN 'Perawan'
				ELSE
					CASE WHEN status='Kawin' THEN  'Menikah'
					ELSE 'Janda'
					END
			END
		END
		AS keterangan_Case,


		CASE jenis_kelamin
			WHEN 'Pria' THEN
				CASE status
					WHEN 'sendiri' THEN 'Perjaka'
					WHEN 'Kawin' THEN 'Menikah'
					ELSE 'Duda'
					END
				
			ELSE
				CASE status
					WHEN 'sendiri' THEN 'Perawan'
					WHEN 'Kawin' THEN 'Menikah'
					ELSE 'Janda'
					END
				END AS ket_Case,

			IF(jenis_kelamin='Pria',
				CASE status
					WHEN 'sendiri' THEN 'Perjaka'
					WHEN 'Kawin' THEN 'Janda'
					ELSE 'Duda'
				END,
				CASE status
					WHEN 'sendiri' THEN 'Perawan'
					WHEN 'Kawin' THEN 'Menikah'
					ELSE 'Janda'
				END
				) AS Keterangan_IF_CASE


FROM vw_seluruh_warga;

/* nilai */

CREATE TABLE IF NOT EXISTS nilai (
	nama_nilai VARCHAR(15),
	teori TINYINT,
	prakter TINYINT,
	PRIMARY KEY (nama_nilai, teori, prakter)
); 


DELETE FROM nilai;


INSERT INTO nilai VALUES
 ('M.Septiawan', 100, 100),
 ('Damayanti', 70, 90),
 ('Dzakiy', 85, 79),
 ('Dzaka', 68, 75),
 ('Chacha', 43, 68);

DROP VIEW vw_nilai_siswa_prosentase_TP;

CREATE VIEW vw_nilai_siswa_prosentase_TP AS
 	SELECT nama_nilai,
 			teori,
 			teori *0.3 AS ProsenTeori,
 			prakter,
 			prakter *0.7 AS ProsenPraktek
 	FROM nilai;

DROP VIEW  vw_nilai_siswa_total;

 CREATE VIEW  vw_nilai_siswa_total AS
 	SELECT	nama_nilai,
 		teori,
 		ProsenTeori,
 		prakter,
 		ProsenPraktek,
 		ProsenTeori + ProsenPraktek AS Total
 	FROM vw_nilai_siswa_prosentase_TP;

DROP VIEW vw_nilai_siswa_grade;

 CREATE VIEW  vw_nilai_siswa_grade AS
 	SELECT nama_nilai,
 			teori,
 			ProsenTeori,
 			prakter,
 			ProsenPraktek,
 			Total,
			IF(Total>90,'A',
				IF(Total>80,'B',
					IF(Total>70,'C',
						IF(Total>50,'D',
							'E'))))
			AS Grade,

			IF(Total>80,'K','BK')
			AS Kompetensi

	FROM vw_nilai_siswa_total;

DROP VIEW vw_keterangan;

 CREATE VIEW vw_keterangan AS
 	SELECT nama_nilai,
 		teori,
 		ProsenTeori,
 		prakter,
 		ProsenPraktek,
 		Total,
 		Grade,
 		Kompetensi,
 		IF(Grade='A','Memuaskan',
 				IF(Grade='B','Baik',
 					IF(Grade='C','Cukup',
 						'Kurang')))
 			AS Keterangan
 	FROM vw_nilai_siswa_grade;

-- Nilai Siswa Keterangan

DROP VIEW vw_nilai_siswa_keterangan;

CREATE VIEW vw_nilai_siswa_keterangan AS
	SELECT nama_nilai,
 		teori,
 		ProsenTeori,
 		prakter,
 		ProsenPraktek,
 		Total,
 		Grade,
 		Kompetensi,
 		CASE Grade
 			WHEN 'A' THEN 'Memuaskan'
 			WHEN 'B' THEN 'Baik'
 			WHEN 'C' THEN 'Cukup'
 			ELSE 'Kurang'
 			END
 			AS SiswaKeterangan
 	FROM vw_keterangan;

/* hasil grade */

DROP VIEW vw_hasilgrade;

CREATE VIEW vw_hasilgrade AS
SELECT Grade,

COUNT(Grade) as hasil_Grade FROM vw_nilai_siswa_grade
GROUP BY Grade
ORDER BY Grade ASC;

/* TOTAL KOMPETEN */
DROP VIEW vw_total_kompetensi;

CREATE VIEW vw_total_kompetensi AS
SELECT Kompetensi,

COUNT(Kompetensi) as Total_Kompetensi FROM  vw_nilai_siswa_grade
GROUP BY Kompetensi
ORDER BY Kompetensi ASC;

DROP VIEW vw_total_keterangan;

CREATE VIEW vw_total_keterangan AS
SELECT SiswaKeterangan,

COUNT(SiswaKeterangan) AS total_SiswaKeterangan FROM vw_nilai_siswa_keterangan
GROUP BY SiswaKeterangan;

DROP VIEW vw_totalgrade;

CREATE VIEW vw_totalgrade AS
SELECT Grade AS Nama_Grade,
		COUNT(Grade) AS Jumlah_Siswa 
FROM vw_nilai_siswa_keterangan
WHERE Kompetensi = 'K' and SiswaKeterangan = 'Memuaskan'
GROUP BY Grade;



